...
- Check where the MBC and qCO2 formulas (Section 8) come from, and
  confirm their units
- Update the example dataset to include more plates (e.g. plates 11-15,
  for 10 total across 2 runs), so the `run_id` parameter demonstrates
  meaningfully rather than showing a single run
- Reevaluate which variables `MR_indices` keeps, for readability
  (Section 9)
- Section 10’s standard-soil QC plot only shows one run currently —
...
- Investigate and resolve the one dropped data point warning in `rSIRs`
  (Section 9) — likely a negative-or-zero value going into
