...
# replace "extr" by "extr_1", and create "extr_2" to create plate that artificially has 2 extractants
...
tidy_extr_1 <- dbl_extr_plate |> dplyr::filter(extr_id == "extr_1")  
tidy_extr_2 <- dbl_extr_plate |> dplyr::filter(extr_id == "extr_2")  
...
#(extr_avg <- dplyr::bind_rows(extr_1_avg, extr_2_avg))
```
