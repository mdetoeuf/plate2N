...
- Investigate and resolve the one dropped data point warning in `rSIRs` (Section 7.2) — likely a negative-or-zero value going into `log()`
- change ylim() in normalized / not normalized plot to reflect new data (5 plates added) so no points are out of range
...
- **Section 4**: compute a preliminary average, then iteratively check for and remove per-plate[^1]/substrate outliers, and recompute the final average.
...
# 2 - Import & tidy 2+ layers, then join metadata {#sec-import-tidy-metadata}
...
# 3 - Clean data, normalize & convert to CO2 rate {#sec-clean-normalize-convert}
...
## 3.2 - Remove "empty" and NAs {#sec-remove-empty}
...
# 4 - Average per plate & substrate / outlier removal {#sec-average-outlier-removal}
...
# 5 - Correction of averages - remove basal respiration {#sec-basal-respiration}
...
# 6 - QC - check standard soils {#sec-qc-std-soils}
...
# 7 - Compute indices & final wrangling {#sec-compute-indices}
