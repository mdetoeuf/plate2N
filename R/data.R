...
#'
#' metadata is an example data set of the package plate2N. Must-have columns are
#'    those detailed here. Additional recommanded columns are those recording detail on extractant.
...
#'    Here, we propose to record extractant species (K2SO4), extractant concentration (0.5) and unit (M).
#'
#' More experimental parameters could be relevant as well, anything that is valid
#'    on a per-plate basis (e.g., dilution, incubation time,
#'    sampling time, researcher, machine used, wavelength,...)
#'
#' Recording date and time can help to distinguish batch-effects
#'
#' You can create your own metadata as a csv and import it using a family of
#'    import functions from the readr package (e.g., `readr::read_csv()` or `readr::read_csv2()`).
#'
#' @format
...
#'
#' This is a modified version of `tidy_plates` where some wells have been artificially
#'     attributed to a second extractant. It serves as example to test several steps
#'     of the vignette `blank-correction` and functions therein. It has one more column
#'     than `tidy_plates` that stores the information of which well needs to be
#'     corrected with which extractant
#'
#' @format
#' A tibble similar to `tidy_plates`, with one row per well, and the following columns:
#'
#' \describe{
#'  \item{row,column,well_id,unique_well_id,dataset,plate_id}{allowing the unique
#'      identification of each well of the 96-well plate}
#'  \item{map}{identifier of samples, Standard curve or extractant (“extr_1" or "extr_2")}
#'  \item{abs}{raw absorbance read of each well}
#'  \item{extr_id}{extractant identifier corresponding to each well. 3 possible
#'      values: "extr_1", "extr_2" or "none" (for empty wells or standard curves)}
##' }
#'
